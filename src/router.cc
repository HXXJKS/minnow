...
void Router::route()
{
  ...
  for (auto &tmp_interface : _interfaces)
  {
    auto &tmp_dgrams = tmp_interface->datagrams_received();
    ...
    while (!tmp_dgrams.empty())
    {
      ...
      {
        // if find prefix
        if (router_table[i].second.pre_len == 0 || ((router_table[i].first ^ tmp_dgram.header.dst) >> (static_cast<uint8_t>(32) - router_table[i].second.pre_len)) == 0)
        {   

          if (tmp_idx == -1 ||
            router_table[i].second.pre_len > tmp_longest_pre_len)
          {
              tmp_longest_pre_len = router_table[i].second.pre_len;
              tmp_idx = i;
          }
        }
      }
      ...
      if (tmp_idx != -1)
      {
        ...
        if (tmp_dgram.header.ttl > 1)
        {
          ...
          tmp_dgram.header.compute_checksum();
          // send the datagram
